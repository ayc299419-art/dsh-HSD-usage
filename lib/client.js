...
(() => {
    ...
    function mountPanel(options = {}) {
      ...
      if (typeof document !== "undefined") {
        ...
        try {
          ...
          // 跨客户端同步：从宿主拉取权威配置并应用（实例 + 全局配置）
          storage.hydrate().then((adopted) => {
            if (adopted && typeof adopted === "object") {
              if (adopted.config && typeof adopted.config === "object") {
                try { appCfg.controller.updateConfig(adopted.config); } catch (e) {}
              }
              if (Array.isArray(adopted.instances)) {
                try {
                  appCfg.manager.instances = adopted.instances;
                  appCfg.manager.persist();
                } catch (e) {}
              }
              eventBus.emit("instances-changed", appCfg.manager.getAll());
              eventBus.emit("config-changed", appCfg.controller.config);
            }
          }).catch(() => {});
        } catch (err) {
  ...
  register("utils/time.js", "utils", function(require, module, exports) {
    ...
    exports.formatFull = formatFull;
      ...
      console.log("[dsh-HSD-usage] factory 执行完成 rev=91e9309cb3");
      try { if (typeof window !== "undefined" && window.__usageMonitor) window.__usageMonitor.buildRev = "91e9309cb3"; } catch {}
